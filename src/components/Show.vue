<template>
  <div>
      <!-- 
        file_create_time: "2020-11-11-11-2-16"
        file_label: 0
        file_name: "Access_Rights_for_automation_test"
        file_path: "Access_Rights_for_automation_test.url"
        file_size: 0
        file_type: 1
        file_update_time: "2020-11-11-11-2-17"
        file_video_length: ""
        id: 1
        is_delete: 0
       -->
...
export default {
    ...
    methods: {
        ...
        get_file_data(){
            // console.log(JSON.parse(localStorage.getItem('search_file')))
            ...
            if(type_id){
                for(let i = 0;i < this.file_type_list.length;i++){
                    if(this.file_type_list[i].id == type_id){
                        // console.log(this.file_type_list[i].file_type)
        ...
        //     console.log(column)
        //     let label_id_list = row.file_label;
        //     let label_list = [];
        //     if(label_id){
        //         for(let i = 0;i < this.file_label_list.length;i++){
        //             for(let j = 0;j < this.label_id_list;j++){
        //                 if(this.file_label_list[i].id == label_id){
        //                     return this.file_label_list[i].label
        //                 }
        //             }
        //         }
        ...
        formatterVideo(row, column) {
            ...
            if(file_size < 60){
                ...
                return (file_size/60) + '分钟' + (file_size%60) + '秒'
            }else{
                return (file_size/3600) + '小时' + (file_size % 3600)/60 + '分钟' + (file_size % 3600)%60 + '秒'
            }
        }
    }
